...
class SmallObjectPriorBranch(nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        if (self.training or getattr(self, "force_aux", False)) and getattr(self, "return_aux", False):
            ...
            for name, layer_idx in aux_indices.items():
                ...
                if feat is None:
                    print(f"[AUX] Warning: {name} at layer {layer_idx} not captured.")
                    continue
                aux_map = self.sopb[name](feat)
                aux_feats.append(feat)
                aux_maps.append(aux_map)
                print(f"[AUX] {name} shape={tuple(feat.shape)}, heatmap shape={tuple(aux_map.shape)}")
            aux_outputs = {"maps": aux_maps, "features": aux_feats}
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        self.aux_indices = {
            ...
            "heatmap3": 6,   # B3
            "heatmap4": 8,   # B4
            "heatmap5": 20,  # B5, final high-level feature before Detect
        }
        self.aux_channels = {
            "heatmap1": 64,
            "heatmap2": 128,
        ...
        aux_layer_setting = os.getenv("YOLO_AUX_LAYERS", "all").lower()
        if aux_layer_setting == "all":
            self.active_aux_names = set(self.aux_indices)
        else:
            self.active_aux_names = set()
            for name in aux_layer_setting.split(","):
                name = name.strip()
                if not name:
                    continue
                name = aux_layer_aliases.get(name, name)
                if name not in self.aux_indices:
                    raise ValueError(f"Unsupported YOLO_AUX_LAYERS item={name!r}, expected all, b1-b5, or heatmap1-heatmap5.")
                self.active_aux_names.add(name)

        self.sopb = nn.ModuleDict(
            {
                name: SmallObjectPriorBranch(in_channels=channels, hidden_channels=64)
                for name, channels in self.aux_channels.items()
            }
        )
        self.return_aux = False
        self.force_aux = False
        self._aux_map = None
        # ============================================== debug: print layer index / type / shape ================================================
        ...
        if verbose:
            self.info()
            LOGGER.info("")
        self.return_aux = os.getenv("YOLO_AUX_EXPERIMENT", "exp1_sum").lower() != "off"
