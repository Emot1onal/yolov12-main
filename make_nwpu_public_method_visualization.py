...
class Predictor:
    ...
    def predict(self, image_path, imgsz):
        if self.kind == "yolo":
            return predict_yolo(self.model, image_path, self.conf, imgsz)
        if self.kind == "torchvision":
            return predict_torchvision(self.model, image_path, self.conf)
        raise ValueError(f"Unknown predictor kind: {self.kind}")


def parse_args():
    parser = argparse.ArgumentParser(
        description="Create a paper-style NWPU visualization with public methods, YOLOv12, and HARPNet."
    )
    parser.add_argument(
        "--data",
        type=Path,
        default=Path(r"C:\Users\14288\OneDrive\Desktop\Dataset1-NWPU\NWPU_VHR10_YOLO\data.yaml"),
    )
    parser.add_argument("--split", default="test")
    parser.add_argument("--out", type=Path, default=Path(r"..\runs\paper_figures\nwpu_public_methods_grid.png"))
    ...
    parser.add_argument("--images", nargs="*", default=None, help="Optional fixed image filenames, e.g. 149.jpg 192.jpg")
    parser.add_argument("--no-public", action="store_true", help="Only draw GT, YOLOv12, and HARPNet.")
    parser.add_argument("--skip-fasterrcnn", action="store_true")
    parser.add_argument("--skip-maskrcnn", action="store_true")
    parser.add_argument("--skip-yolov8-public", action="store_true")
    parser.add_argument(
        "--yolov12",
        type=Path,
        default=Path(r"..\runs\detect\off_all\weights\best.pt"),
        help="YOLOv12 baseline weights.",
    )
    parser.add_argument(
        "--ours",
        type=Path,
        default=Path(r"..\runs\detect\exp2_sum_all\weights\best.pt"),
        help="HARPNet weights.",
    )
...
def read_data_yaml(path):
    ...
    root = Path(data["path"])
    names = data.get("names", NWPU_NAMES)
...
def draw_boxes(image, boxes, names, normalized=False, show_score=True):
    ...
    fnt = get_font(max(10, im.width // 42))
    w, h = im.size
    for box in boxes:
        if normalized:
            cls, xc, yc, bw, bh = box
...
def make_method_grid(rows, image_names, cells, cell_size, out):
    label_w = 116
    top_h = 18
    bottom_h = 32
    gap = 10
    ...
    for r, row_name in enumerate(rows):
        ...
        for c, _ in enumerate(image_names):
...
def predict_yolo(model, image_path, conf, imgsz):
...
def predict_torchvision(model, image_path, conf):
    import torch
...
def score_image(gt_boxes, public_stats, base_stats, ours_stats):
    ...
    score += (ours_stats["tp"] - public_best_tp) * 7.0
    score += (base_stats["fn"] - ours_stats["fn"]) * 8.0
    score += (public_best_fp - ours_stats["fp"]) * 1.2
    score += (ours_stats["count"] - base_stats["count"]) * 0.7
    score += ours_stats["conf_sum"] * 0.2
    score += min(len(gt_boxes), 8) * 0.4
    return score


def select_images(all_images, predictors, num_images, seed, scan_limit, iou_thr, fixed_images):
    if fixed_images:
    ...
    for idx, image_path in enumerate(candidates, start=1):
        ...
        for pred in public:
            try:
                public_stats.append(detection_stats(gt, pred.predict(image_path, 640), iou_thr))
            except Exception as exc:
                print(f"WARNING: skipped {pred.name} on {image_path.name}: {exc}")
        base_stats = detection_stats(gt, base.predict(image_path, 640), iou_thr)
        ours_stats = detection_stats(gt, ours.predict(image_path, 640), iou_thr)
...
def hf_download_first(repo_id, candidates):
    ...
    model_files = sorted(f for f in files if f.endswith((".pth", ".pt")))
    if model_files:
        return hf_download(repo_id, model_files[0])
    raise FileNotFoundError(f"No .pth/.pt weights found in {repo_id}. Files: {sorted(files)[:20]}")
...
def load_yolo_predictor(label, weight_path, conf):
    ...
    return Predictor(label, "yolo", YOLO(str(weight_path)), conf)


def load_public_yolov8(conf):
    from ultralytics import YOLO

    errors = []
    for repo_id in (
        "bluelabel/satellite-equipment-detection-yolov8n-vhr10",
        "omgbobbyg/satellite-equipment-detection-yolov8n-vhr10",
    ):
        try:
            try:
                path = hf_download(repo_id, "best.pt", local_files_only=True)
            except Exception:
                path = hf_download(repo_id, "best.pt")
            return Predictor("YOLOv8n-VHR10", "yolo", YOLO(str(path)), conf)
        except Exception as exc:
            errors.append(f"{repo_id}: {exc}")
    raise RuntimeError("Could not load public YOLOv8n-VHR10:\n" + "\n".join(errors))


def load_torchvision_detector(label, repo_id, architecture, conf):
    import torch
    from torchvision.models.detection import fasterrcnn_resnet50_fpn_v2, maskrcnn_resnet50_fpn

    path = hf_download_first(repo_id, ("best_model.pth", "model.pth", "detector.pth", "pytorch_model.pth"))
    if architecture == "fasterrcnn_resnet50_fpn_v2":
        model = fasterrcnn_resnet50_fpn_v2(weights=None, weights_backbone=None, num_classes=11)
    elif architecture == "maskrcnn_resnet50_fpn":
        model = maskrcnn_resnet50_fpn(weights=None, weights_backbone=None, num_classes=11)
    else:
    ...
    return Predictor(label, "torchvision", model, conf)
...
def build_predictors(args):
    ...
    if not args.no_public:
        if not args.skip_fasterrcnn:
            try:
                predictors.append(
                    load_torchvision_detector(
                        "Faster R-CNN",
                        "giswqs/nwpu-vhr10-fasterrcnn",
                        "fasterrcnn_resnet50_fpn_v2",
                        args.public_conf,
                    )
                )
            except Exception as exc:
                print(f"WARNING: skipped Faster R-CNN: {exc}")
        if not args.skip_maskrcnn:
            try:
                predictors.append(
                    load_torchvision_detector(
                        "Mask R-CNN",
                        "giswqs/nwpu-vhr10-maskrcnn",
                        "maskrcnn_resnet50_fpn",
                        args.public_conf,
                    )
                )
            except Exception as exc:
                print(f"WARNING: skipped Mask R-CNN: {exc}")
        if not args.skip_yolov8_public:
            try:
                predictors.append(load_public_yolov8(args.public_conf))
            except Exception as exc:
                print(f"WARNING: skipped YOLOv8n-VHR10: {exc}")
...
def main():
    ...
    selected = select_images(images, predictors, args.num_images, args.seed, args.scan_limit, args.iou, args.images)
